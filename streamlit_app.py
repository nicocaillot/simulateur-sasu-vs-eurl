...
is_sasu = max(0, resultat_sasu * taux_is)
...
total_net_eurl = revenu_net_eurl
...
with col1:
    ...
    st.write(f"Rémunération nette : **{revenu_net_sasu:.0f} €**")
    st.write(f"Dividendes nets : **{revenu_dividende_net:.0f} €**")
    st.write(f"🟢 Revenu total net : **{total_net_sasu:.0f} €**")
...
with col2:
    ...
    st.write(f"🟢 Revenu total net : **{total_net_eurl:.0f} €**")
...
if st.button("📄 Exporter en PDF"):
    ...
    pdf.set_font("Arial", "", 12)
    pdf.cell(0, 10, f"Revenu net SASU : {total_net_sasu:.0f} €", ln=1)

    pdf.cell(0, 10, f"  - Résultat : {resultat_sasu:.0f} €", ln=1)
    pdf.cell(0, 10, f"  - IS : {is_sasu:.0f} €", ln=1)
    pdf.cell(0, 10, f"  - Cotisations : {cot_sasu:.0f} €", ln=1)
    pdf.cell(0, 10, f"  - Dividendes nets : {revenu_dividende_net:.0f} €", ln=1)

    pdf.ln(5)
    pdf.set_font("Arial", "B", 14)
    pdf.cell(0, 10, "EURL :", ln=1)
    pdf.set_font("Arial", "", 12)
    pdf.cell(0, 10, f"Revenu net EURL : {total_net_eurl:.0f} €", ln=1)

    pdf.cell(0, 10, f"  - Résultat : {resultat_eurl:.0f} €", ln=1)
    pdf.cell(0, 10, f"  - IR : {ir_eurl:.0f} €", ln=1)
    ...
    if diff != 0:
        ...
        pdf.cell(0, 10, f"{meilleure} est plus avantageuse de {abs(diff):.0f} €", ln=1)

    pdf_output = BytesIO()
    pdf.output(pdf_output)
    st.download_button(label="📥 Télécharger le PDF", data=pdf_output.getvalue(), file_name="simulation-sasu-vs-eurl.pdf", mime="application/pdf")